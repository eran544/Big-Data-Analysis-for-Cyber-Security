...
lambda x: x.replace(hour=0, minute=0, second=0))
        ...
        # Clean-Prevalent file (more than X machines)
        if (daySet[i]['DailyMachineCount'].sum() > 5):
            daySet[i]['MoreThan5'] = True
        else:
            daySet[i]['MoreThan5'] = False
...
fileML = open("All_data_to_Hour_Day_datasets timestamp.txt", "a+")  # append mode
